...
def calculate_rsi(prices, period=14):
    """Calcula RSI con suavizado de Wilder (EWM) para paridad científica."""
    delta = prices.diff()
...
def calculate_bollinger_bands(prices, period=20, std_dev=2):
    """Calcula Bollinger con precisión ddof=0."""
...
def calculate_confluence_score(p, rsi, bb_u, bb_l, ema_200, usdt_d=8.0, side="LONG", elliott=""):
    """Calcula score con bonus Elliott para paridad con bot real."""
    score = 0
    if side == "LONG":
        if rsi <= 30: score += 2
        elif rsi <= 40: score += 1
    ...
    if (side == "LONG" and bb_l and p <= bb_l * 1.01) or (side == "SHORT" and bb_u and p >= bb_u * 0.99): score += 1
...
def calculate_atr_series(df, period=14):
    """Calcula ATR con suavizado de Wilder (EWM)."""
...
def run_detailed_backtest(symbol="ETH/USDT", target_date_str=None, days_ago=1, version="V1-TECH"):
    ...
    for ts, row in df_test.iterrows():
        ...
        if not active_trade:
            ...
            # Elliott Wave Detection (Lookback logic matching bot)
            # Extraemos una ventana de 50 velas previas en cada punto de la simulación
            df_lookback = df[df.index <= ts].tail(50).copy()
            elliott = indicators.detect_elliot_wave(df_lookback)
            
            # --- FILTRO V1.6.1 AUDIT: REDUCCIÓN DE RUIDO ---
            if "Corrección" in elliott or "Correctiva" in elliott:
                continue

            # — ENTRADA SHORT — (Trend + BB + RSI + Volume V1.6.1)
            if macro_status != "BULL":
                if price < ema_200 and rsi >= rsi_short and bb_status == "🔴 Cerca Banda Superior" and vol > vol_sma:
                    ...
                    conf_score = calculate_confluence_score(price, rsi, bb_u_val, bb_l_val, ema_200, 8.0, "SHORT", elliott)
            ...
            # — ENTRADA LONG — (Trend + BB + RSI + Volume V1.6.1)
            elif macro_status != "BEAR":
                if price > ema_200 and rsi <= rsi_long and bb_status == "🟢 Cerca Banda Inferior" and vol > vol_sma:
                    sl_dist = max(atr * 1.5, price * 0.005)
                    sl = round(price - sl_dist, 2)
                    tp1 = round(price + (sl_dist * 2.0), 2)
                    tp2 = round(price + (sl_dist * 3.0), 2)
                    conf_score = calculate_confluence_score(price, rsi, bb_u_val, bb_l_val, ema_200, 8.0, "LONG", elliott)
